...
export default function BoardPage() {
  ...
  const [activeBoardType, setActiveBoardType] = useState<BoardType | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const navigate = useNavigate();

  const fetchPosts = useCallback(async (boardType: BoardType | null) => {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '2rem auto', padding: '1rem' }}>
      ...
      <button
        ...
        style={{ marginBottom: '1rem', padding: '0.75rem 1rem', cursor: 'pointer' }}
      >
        글쓰기
      </button>
        ...
        <div style={{ display: 'grid', gap: '1rem' }}>
          {posts.map((post) => (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', fontSize: 14, color: '#555' }}>
